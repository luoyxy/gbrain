...
以确切措辞捕获用户的原创想法，具有深度上下文和交叉链接，以便原创文件夹成为 brain 中价值最高的内容。
...
capture_idea(message_text, source_context):
  ...
  if user_generated_the_idea(message_text):
    ...
    destination = "brain/originals/{slug}.md"  # 综合 IS 原创
...
1. **综合 IS 原创。** 当用户以新方式连接两个现有想法时，该综合属于 `brain/originals/`，而不是 `brain/concepts/`。即使组件想法不是新的，新颖的组合也是洞察力。
...
1. **生成一个想法并检查页面。** 在对话中说一些原创的东西（例如，"如果 Markdown 文件实际上是分布式软件会怎样？"）。验证 `brain/originals/{slug}.md` 是使用你的确切措辞创建的，而不是释义。
