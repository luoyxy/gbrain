...
分块（3 种策略，可配置）
  ...
  └── LLM 引导：Claude Haiku 识别 128 词候选中的主题转移
  ↓
嵌入（OpenAI text-embedding-3-large，1536 维度）
  → 批量 100，指数退避，如果失败则为非致命
  ↓
...
GBrain 使用倒数排名融合（RRF）来合并向量和关键词搜索：
...
扩展（可选：Claude Haiku 生成 2 个替代措辞）
  ...
  └── 关键词搜索（PostgreSQL tsvector，ts_rank）
        → 2 倍限制结果
  ↓
RRF 合并（score = Σ(1/(60 + rank))，公平地平衡两者）
  ↓
4 层去重
  ├── 每页最佳 3 块（来源去重）
...
| `src/core/embedding.ts` | OpenAI 嵌入，带批量、重试、退避 |
| `src/core/chunkers/recursive.ts` | 基础分块器（300w，5 级分隔符） |
...
- **page_versions** — 用于差异/还原的快照历史
- **raw_data** — 来自外部 API 的侧车 JSON（保留来源）
- **files** — 存储后端中的二进制附件
- **ingest_log** — 导入操作的审计跟踪
- **config** — 大脑级设置（版本、嵌入模型、分块策略）

全文搜索使用加权 tsvector：title（A）、compiled_truth（B）、timeline（C）。
向量搜索在 content_chunks.embedding 上使用带余弦距离的 HNSW 索引。

## 薄工具架原则

GBrain 是确定性层。技能和配方是潜在空间层。

有关完整的架构哲学，请参阅[薄工具架，胖技能](../ethos/THIN_HARNESS_FAT_SKILLS.md)。

- **GBrain CLI** = 薄工具架（相同输入 → 相同输出）
- **技能**（摄取、查询、维护、丰富、简报、迁移、设置）= 胖技能
- **配方**（voice-to-brain、email-to-brain）= 安装基础设施的胖技能
